import { PROJECT_23NOV_BLOCKCHIAN_backend } from "../../declarations/PROJECT_23NOV_BLOCKCHIAN_backend";
...
appointForm.addEventListener("submit", async (e) => {
  ...
  try {
    const getId = await PROJECT_23NOV_BLOCKCHIAN_backend.addTodo(obj.textarea);
...
window.addEventListener("DOMContentLoaded", async () => {
  try {
    const getAll = await PROJECT_23NOV_BLOCKCHIAN_backend.getAllTodo();
...
async function deleteTodo(userId) {
  try {
    ...
    await PROJECT_23NOV_BLOCKCHIAN_backend.deleteTodo(userId);
